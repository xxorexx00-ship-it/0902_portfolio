...
{
  id: 'kawagoe-premium-map',
  title: '小江戸・川越 観光散策マップ',
  categories: ['web-design'],
  categoryLabel: 'Web Design',
  tags: ['HTML', 'CSS', 'GSAP', 'ScrollTrigger', 'Horizontal Scroll'],
  description: `【制作コンセプト・デザイン意図】
世界観とブランドの表現
川越が持つ「歴史の重厚感」や「上質な和の雰囲気」を最高クオリティで表現するため、単なる観光地の紹介ページではなく、ハイエンドなブランドLPのようなシネマティックで洗練された空気感を目指しました。

伝統とモダンさの融合
明朝体や繊細なトラッキング（文字間隔）、できるだけオーバーレイに頼らず写真をクリアに見せる引き算のデザインを採用。太く大きめのタイトルの存在感と大胆な余白を掛け合わせることで、現代的でラグジュアリーなWebデザインに仕上げています。

【実装上の工夫と技術的ポイント】
GSAPとScrollTriggerによるシームレスな横スクロール
縦スクロールの動きを途中で固定（pin）し、横方向へのダイナミックなスクロールへと変換。ユーザーの没入感を高め、誌面をめくるような心地よいインタラクションを実装しています。

clip-path（polygon）による独自の視覚リズム
CSSの clip-path を活用し、各パネルのカードを斜めにカットした独自のシルエットに統一。グリッドの単調さをなくし、スタイリッシュなビジュアルリズムを生み出しています。

【意識したユーザー体験（UX）】
情報の一貫性と没入感
ヒーローヘッダーから導入、横スクロールの各スポット、そしてラストの結びまで、すべてのセクションでトーン＆マナー（黒と上質な光、洗練された写真表現）を徹底。ノイズを削ぎ落とし、川越の街を巡る特別な旅のストーリーに深く浸れる構成に仕上げています。`,
  thumbnail: 'images/works/kawagoe.webp',
